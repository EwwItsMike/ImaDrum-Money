...
//            audioPlayer.play(buf, gain);
        ...
        // ^ this does NOT work, and will throw the same error with message "mark/reset not supported"
        // Leaving this in as comments in case anyone runs into the same problem and checks this code for answers
        try {
            float gain = 20f * (float)Math.log(config.soundVolume() / 100f);
            audioPlayer.play(audioPlayer.getClass(), sound.getFileName(), gain);
